...
int main() {
    std::cout << "===========================================\n";
    std::cout << "  SLOW Controller - Low Control Effort\n";
    std::cout << "===========================================\n\n";
    ...
    sf::RenderWindow window(sf::VideoMode(sf::Vector2u(1200, 800)), "Rocket Sim - SLOW Controller");
    window.setFramerateLimit(60);

    sf::Font font("C:\\Windows\\Fonts\\consola.ttf");

    Controller controller(100.0, M_PI / 2);
    if (!controller.load_gains_from_json("../lqr_gains_slow.json")) {
        std::cerr << "[!] Could not load lqr_gains_slow.json\n";
        return 1;
    }
    ...
    std::cout << "\n[+] Slow sim ended. Time: " << simulation_time << "s, Frames: " << frame << "\n";
    return 0;
}
